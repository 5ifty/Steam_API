...
class Steam():
  def __init__(self):
    ...
    self.key = ""   # Enter API key here (Without doing so you will 100% get a 401 or 404 error)
    if self.key == "":
    ...
    steamID = input("Please enter a valid Steam64 ID, (NOTE: any other form of ID will not work) \n")



    resp = requests.get(self.url+steamID)
    ...
    if onstatus == 1:
      bar = "Online"
    elif onstatus == [2, 3, 4]:    # SteamAPI gives variable online status, this sorts the value of key into a response
    ...
    if 'yes' in foo:
        ...
        print("\n\n\nThank you for using the SteamAPI console made by 5ifty")
        time.sleep(5)
        exit()
